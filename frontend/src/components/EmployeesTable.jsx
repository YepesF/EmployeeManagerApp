...
function EmployeesTable() {
  ...
  const { page, limit, pages } = state.employees;
  ...
  const showModal = (employeeData) => {
    ...
    document.getElementById('edit_employee').showModal();
  };
  ...
  const handleDelete = async (requestId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        deleteEmployee(token, requestId)
          .then(() => {
            if (state.requests.page === 1) {
              getEmployees(state.token).then((payload) => {
                dispatch({ type: 'UPDATE_EMPLOYEES', payload });
              });
            }
            Swal.fire({
          ...
          .catch(() => {
            Swal.fire({
              title: 'Error',
              text: 'No se pudo eliminar el empleado, intente de nuevo.',
              icon: 'error',
            });
          });
      }
  ...
  return (
    <div className="flex min-h-[35rem] flex-col justify-between overflow-x-auto">
      ...
      <table className="table table-zebra table-xs 2xl:table-md">
        <thead>
          <tr>
            <th></th>
            <th>Nombre</th>
        ...
        <tbody>
          {employees.map((employee, index) => (
            <tr key={employee.name + index}>
              ...
              <td>{new Date(employee.hireDate).toLocaleString()}</td>
